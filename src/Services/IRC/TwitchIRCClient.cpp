...
#include "Services/IRC/TwitchIRCClient.hpp"
...
namespace ChatLib::Services
{
    std::vector<std::string> split(std::string const& text, char sep)
    {
        ...
        tokens.push_back(text.substr(start, end - 1));
        return tokens;
    }
    ...
    {
        std::string buffer = _socket.RecieveData();
    ...
    void TwitchIRCClient::CallHook(std::string command, IRCMessage message)
    {
        ...
        for (std::list<ChatLib::Services::IRCCommandHook>::const_iterator itr = _hooks.begin(); itr != _hooks.end(); ++itr)
        {
